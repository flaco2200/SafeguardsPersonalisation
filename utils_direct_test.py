...
class ContentClassifier:
    ...
    def evaluate(self, data_loader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch_idx, (batch_input_ids, batch_attention_mask, batch_labels) in enumerate(data_loader):
                ...
                for j in range(len(batch_input_ids)):
                    ...
                    if offensive_probs[j] >= self.config.decision_threshold:
                        predicted_label = 1
                        coverage += 1
                    elif (1 - offensive_probs[j]) >= self.config.decision_threshold:
                        predicted_label = 0
                        coverage +=1
                    else:
                        abstain += 1
        ...
        metrics = self._calculate_metrics(true_positives, false_positives, false_negatives, coverage)
        return metrics

    def _calculate_metrics(self, tp: int, fp: int, fn: int, coverage: int) -> Dict[str, float]:
...
class MetricsLogger:
    ...
    def __init__(self, base_dir: str = "metrics_logs"):
        ...
        self.metrics_dir = self.base_dir / "direct_performance_metrics"
